...
exports.evaluateCandidateUnderwritingRules = evaluateCandidateUnderwritingRules;
exports.evaluateUnderwritingRules = evaluateUnderwritingRules;
exports.buildUnderwritingRuleSnapshot = buildUnderwritingRuleSnapshot;
exports.evaluateUnderwritingRuleDrift = evaluateUnderwritingRuleDrift;
...
function resolveCarrierId(familyId) {
    const normalized = (familyId || "").toLowerCase();
    if (normalized.includes("markel"))
        return "carrier-markel";
    if (normalized.includes("ana") || normalized.includes("anac"))
        return "carrier-anac";
    if (normalized.includes("contractor") || normalized.includes("supplement"))
        return "carrier-markel";
    return "carrier-default";
}
function createRuleSet(carrierId) {
    const shared = [
        {
        ...
        },
        {
            ...
            predicate: ({ candidate, blockText }) => !blockText.toLowerCase().includes("policy") || candidate.acordCode.toLowerCase().includes("policy"),
            scoreImpact: 0.06,
        },
    ];
    if (carrierId === "carrier-markel") {
        shared.push({
            ...
            predicate: ({ candidate, blockText }) => !blockText.toLowerCase().includes("subcontract") ||
    ...
    if (carrierId === "carrier-anac") {
        shared.push({
            ...
            predicate: ({ candidate, blockText }) => !blockText.toLowerCase().includes("license") ||
...
function applyRuleOverride(evaluation, override) {
    if (!override)
        return evaluation;
    return {
        ...evaluation,
...
function evaluateCandidateUnderwritingRules(candidate, options) {
    ...
    const evaluations = rules.map((rule) => {
        ...
        const override = (options.overrides || []).find((item) => item.ruleId === rule.ruleId &&
    ...
    const scoreDelta = toFixed(evaluations.reduce((sum, evaluation) => sum + evaluation.scoreImpact, 0));
    return {
        scoreDelta,
        hardFailures,
        evaluations,
...
function evaluateUnderwritingRules(payload, options) {
    ...
    const conflicts = evaluations
        ...
        severity: result.severity === "blocker" ? "blocker" : "warning",
        lineage: result.lineage,
    })))
        .sort((left, right) => left.extractionBlockId.localeCompare(right.extractionBlockId) || left.ruleId.localeCompare(right.ruleId));
    const rulesHash = hashString(stableSerialize(createRuleSet(carrierId).map((rule) => ({
        ruleId: rule.ruleId,
        carrierId: rule.carrierId,
        kind: rule.kind,
        scoreImpact: rule.scoreImpact,
    }))));
    const outcomeHash = hashString(stableSerialize(evaluations.map((item) => ({
        extractionBlockId: item.extractionBlockId,
...
function buildUnderwritingRuleSnapshot(payload, options) {
...
function evaluateUnderwritingRuleDrift(baseline, current) {
    if (!baseline) {
        return {
            hasDrift: false,

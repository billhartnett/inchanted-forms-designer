import type {
  AcordLabelCandidate,
  GlobalSemanticGraphEdge,
  GlobalSemanticGraphEdgeOverride,
  GlobalSemanticGraphMergeDecision,
  GlobalSemanticGraphNode,
  GlobalSemanticGraphSnapshot,
  SemanticMemorySnapshot,
} from "../acord";
import { getAcordOntology } from "../acord/ontology.ts";
import { selectOntologyBundlesForFamily } from "../acord/multiOntology.ts";
import type { MappingPersistencePayload } from "../types";
import { evaluateSemanticFusion, getCrossFormUnificationGraph } from "./semanticFusion.ts";
import { buildSemanticMemorySnapshot } from "./semanticMemory.ts";
...
export type GlobalSemanticGraphInference = {
  extractionBlockId: string;
  chosenCode?: string;
  inferredCodes: string[];
  inferredMissingRelationships: string[];
  confidence: number;
...
export type GraphHarmonizationReport = {
  generatedAt: string;
...
export type GlobalSemanticDriftReport = {
  hasDrift: boolean;
  driftHash: string;
  differences: Array<{
    key: string;
...
function nodeId(nodeType: string, namespace: string, code: string): string {
  return `${nodeType}:${namespace}:${code}`;
}

function edgeId(fromNodeId: string, toNodeId: string, edgeType: string): string {
...
function applyGraphDecisions(
  snapshot: GlobalSemanticGraphSnapshot,
  decisions: GlobalSemanticGraphMergeDecision[],
): GlobalSemanticGraphSnapshot {
  if (!decisions.length) {
    return snapshot;
  }
  ...
  return {
    ...
    lineage: mergeLineage([
      ...
      ...sorted.map(
        (decision) =>
          `graph.decision:${decision.action}:${decision.targetVersionId || "none"}`,
      ),
    ]),
  };
}
...
function applyEdgeOverrides(
  ...
  overrides: GlobalSemanticGraphEdgeOverride[],
): GlobalSemanticGraphEdge[] {
  if (!overrides.length) {
    return edges;
  }
  ...
  for (const override of overrides) {
    const resolvedEdgeId =
      override.edgeId || edgeId(override.fromNodeId, override.toNodeId, override.edgeType);
  ...
  return Array.from(byId.values()).sort(
    (left, right) =>
      left.edgeType.localeCompare(right.edgeType) || left.edgeId.localeCompare(right.edgeId),
  );
}

export function buildGraphHarmonizationReport(
  familyId: string | undefined,
  nodes: GlobalSemanticGraphNode[],
  edges: GlobalSemanticGraphEdge[],
): GraphHarmonizationReport {
  ...
  const compatibilityViolations = bundles
    .flatMap((bundle) =>
      bundles
        ...
        .filter(
          (peer) => !bundle.metadata.compatibility.includes(peer.metadata.namespace),
        )
        .map((peer) => `${bundle.metadata.namespace}->${peer.metadata.namespace}`),
    )
    .sort((left, right) => left.localeCompare(right));

  const harmonizationHash = hashString(
    stableSerialize({
      namespaces,
      precedenceOrder,
...
export function buildGlobalSemanticGraph(
  documents: FusionDocument[],
  options?: {
    previousSnapshot?: GlobalSemanticGraphSnapshot;
    memorySnapshot?: SemanticMemorySnapshot;
    edgeOverrides?: GlobalSemanticGraphEdgeOverride[];
    mergeDecisions?: GlobalSemanticGraphMergeDecision[];
  },
): {
  snapshot: GlobalSemanticGraphSnapshot;
  harmonization: GraphHarmonizationReport;
} {
  const fusion = evaluateSemanticFusion(documents, {
    fusionOverrides: documents.flatMap((document) => document.payload.fusionOverrides || []),
  });
  const unification = getCrossFormUnificationGraph();
  const memory =
    options?.memorySnapshot ||
    buildSemanticMemorySnapshot(documents, {
  ...
  const bundles = selectOntologyBundlesForFamily(familyId);
  const acordOntology = getAcordOntology();

  const nodes: GlobalSemanticGraphNode[] = [];
  const edges: GlobalSemanticGraphEdge[] = [];

  const pushNode = (node: GlobalSemanticGraphNode) => {
    nodes.push(node);
  };

  const pushEdge = (edge: GlobalSemanticGraphEdge) => {
    edges.push(edge);
  };
  ...
  const dedupedNodes = Array.from(
    new Map(nodes.map((node) => [node.nodeId, node])).values(),
  ).sort((left, right) => left.nodeId.localeCompare(right.nodeId));

  const dedupedEdges = applyEdgeOverrides(
    Array.from(new Map(edges.map((edge) => [edge.edgeId, edge])).values()).sort((left, right) =>
  ...
  const nextVersion =
    (previous ? Number(previous.versionId.replace("graph-v", "")) || 0 : 0) + 1;
  ...
  const snapshot: GlobalSemanticGraphSnapshot = {
    generatedAt: CANONICAL_GENERATED_AT,
    versionId,
    graphHash,
    harmonizationHash: harmonization.harmonizationHash,
  ...
  return {
    snapshot: applyGraphDecisions(
      snapshot,
      options?.mergeDecisions ||
        documents.flatMap(
          (document) => document.payload.globalSemanticGraphMergeDecisions || [],
        ),
    ),
    harmonization,
  };
...
export function evaluateGraphInference(
  payload: MappingPersistencePayload,
  graphSnapshot: GlobalSemanticGraphSnapshot,
): GlobalSemanticGraphInference[] {
  const byCode = new Map<string, GlobalSemanticGraphNode[]>(
    graphSnapshot.nodes.reduce((acc, node) => {
      const current = acc.get(node.code) || [];
      current.push(node);
      acc.set(node.code, current);
      return acc;
    }, new Map<string, GlobalSemanticGraphNode[]>()),
  );

  const incomingByNode = new Map<string, GlobalSemanticGraphEdge[]>(
    graphSnapshot.edges.reduce((acc, edge) => {
      const current = acc.get(edge.toNodeId) || [];
      current.push(edge);
      acc.set(edge.toNodeId, current);
      return acc;
    }, new Map<string, GlobalSemanticGraphEdge[]>()),
  );
  ...
  return payload.mappings
    .map((record) => {
      ...
      const inferredCodes = uniqueSorted(
        graphNodes
          .flatMap((node) => incomingByNode.get(node.nodeId) || [])
          .filter(
            (edge) =>
              edge.edgeType === "equivalence" ||
      ...
      const inferredMissingRelationships = uniqueSorted(
        graphNodes
          .flatMap((node) => incomingByNode.get(node.nodeId) || [])
          .filter(
            (edge) =>
              edge.edgeType === "sibling-required" || edge.edgeType === "parent-child",
          )
...
export function evaluateGlobalSemanticDrift(
  baseline: GlobalSemanticGraphSnapshot | undefined,
  current: GlobalSemanticGraphSnapshot,
): GlobalSemanticDriftReport {
  if (!baseline) {
    return {
      hasDrift: false,
...
export function inferCandidateFromGlobalGraph(
  candidate: AcordLabelCandidate,
  graphSnapshot: GlobalSemanticGraphSnapshot | undefined,
): AcordLabelCandidate["graphInference"] {
  if (!graphSnapshot) {
    return {
      inferred: false,
      confidence: 0,
    ...
    graphSnapshot.edges
      .filter((edge) =>
        candidateNodes.some((node) =>
          edge.toNodeId === node.nodeId || edge.fromNodeId === node.nodeId,
        ),
      )
      .map((edge) => {
  ...
  const confidence = toFixed(
    clamp01(
      Math.min(1, candidateNodes.length / 3) * 0.5 + Math.min(1, recommendations.length / 4) * 0.5,
    ),
  );

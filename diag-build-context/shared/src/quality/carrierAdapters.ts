import type {
  AcordLabelCandidate,
  CandidateCarrierAdapterMetadata,
  CarrierAdapterBundle,
  CarrierAdapterOverride,
  CarrierAdapterSnapshot,
} from "../acord";
import type { MappingPersistencePayload } from "../types";
import { resolveUnificationForCode } from "./semanticFusion.ts";
...
export type CarrierAdapterMapping = {
...
export type CarrierAdapterDrift = {
  hasDrift: boolean;
  driftHash: string;
  differences: Array<{
    key: string;
...
function createCarrierBundles(): CarrierAdapterBundle[] {
  const bundles: CarrierAdapterBundle[] = [
    {
      ...
      nodes: [
        {
          ...
          equivalentUnificationGroups: [resolveUnificationForCode("Contractors_SubcontractorsPaidAmount").groupId],
        },
        {
          ...
          equivalentUnificationGroups: [resolveUnificationForCode("ContractorsUnderwriting_RemodelWorkPercent").groupId],
        },
      ],
      signature: "",
      ...
      nodes: [
        {
          ...
          equivalentUnificationGroups: [resolveUnificationForCode("ContractorsUnderwriting_LicenseNumberIdentifier").groupId],
        },
        {
          ...
          equivalentUnificationGroups: [resolveUnificationForCode("SuretyLineOfBusiness_JobCostBreakdown_SubcontractorPercent").groupId],
        },
      ],
      signature: "",
      generatedAt: CANONICAL_GENERATED_AT,
...
export function getCarrierAdapterBundles(familyId: string | undefined): CarrierAdapterBundle[] {
...
export function adaptCandidateToCarrier(
  candidate: AcordLabelCandidate,
  familyId: string | undefined,
  overrides?: CarrierAdapterOverride[],
): CandidateCarrierAdapterMetadata {
  ...
  for (const bundle of bundles) {
    for (const node of bundle.nodes) {
      ...
      if (!equivalent && !aliasMatch) continue;

      const override = (overrides || []).find(
        (item) => item.carrierId === bundle.carrierId && item.carrierCode === node.carrierCode,
      );
      const mappedAcordCode = override?.forcedAcordCode || node.equivalentAcordCodes[0] || candidate.acordCode;
      const confidence = clamp01(
        (equivalent ? 0.7 : 0.45) + (aliasMatch ? 0.2 : 0) + bundle.precedence / 1000,
      );
...
export function evaluateCarrierAdapterMappings(
  payload: MappingPersistencePayload,
  options?: {
    familyId?: string;
    overrides?: CarrierAdapterOverride[];
  },
): CarrierAdapterMapping[] {
  ...
  return payload.mappings
    .map((record) => {
      ...
      const candidate =
        record.mapping.suggestions.find((item) => item.acordCode === chosen) || record.mapping.chosen;
    ...
    .filter((item): item is CarrierAdapterMapping => Boolean(item))
    .sort((left, right) => left.extractionBlockId.localeCompare(right.extractionBlockId));
}

export function buildCarrierAdapterSnapshot(
  documents: AdapterDocument[],
  options?: {
    previousSnapshot?: CarrierAdapterSnapshot;
    overrides?: CarrierAdapterOverride[];
  },
): CarrierAdapterSnapshot {
  const mappings = documents.flatMap((document) =>
    evaluateCarrierAdapterMappings(document.payload, {
      familyId: document.payload.formFamily?.familyId,
      overrides: options?.overrides || document.payload.carrierAdapterOverrides || [],
    }),
  );
  const carriers = Array.from(new Set(mappings.map((item) => item.carrierId))).sort((a, b) =>
    a.localeCompare(b),
  );
...
export function evaluateCarrierAdapterDrift(
  baseline: CarrierAdapterSnapshot | undefined,
  current: CarrierAdapterSnapshot,
): CarrierAdapterDrift {
  if (!baseline) {
    return {
      hasDrift: false,

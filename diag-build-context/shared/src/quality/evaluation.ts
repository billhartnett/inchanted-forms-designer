...
} from "../acord/ontology.ts";
import {
  classifyFormFamily as inferFormFamily,
  resolveFamilyCalibration,
  suggestFamilyCalibrationOverrides,
} from "./familyClustering.ts";
import {
  evaluateCrossFamilyNormalization,
  normalizeSignalsForFamily,
} from "./crossFamilyNormalization.ts";
import { evaluateSemanticFusion } from "./semanticFusion.ts";
import {
...
import {
  ...
  extractRiskFactors,
} from "./riskDecisionIntelligence.ts";
import { evaluateSubmissionDrift } from "./submissionIntelligence.ts";

export type FixtureFieldExpectation = {
  id: string;
  expectedAcordCode: string;
  expectedExtractionBlockId?: string;
  expectedTopN?: number;
  textPattern?: string;
};
...
export type FixtureExpectations = {
  fixtureId: string;
  familyId?: string;
  fields: FixtureFieldExpectation[];
};

export type CoverageMetrics = {
  familyId: string;
  normalizationFactor: number;
  crossFamilyConfidenceDelta: number;
  fieldsTotal: number;
  fieldsMapped: number;
  ...
  calibratedTopNAccuracy: number;
  baselineTopNAccuracy: number | null;
  calibratedTopNDelta: number | null;
};

export type MappingFieldQuality = {
  fieldId: string;
  extractionBlockId?: string;
  expectedAcordCode?: string;
  chosenAcordCode?: string;
  chosenConfidence: number;
  appliedThreshold: number;
  topNCandidateCodes: string[];
  exactMatch: boolean;
  topNHit: boolean;
  chosenWithinTopN: boolean;
  chosenRank: number | null;
  associationCorrect: boolean;
  candidateCount: number;
  rationaleDisagreement: boolean;
  ontologyWarnings: string[];
  ontologyConstraintViolations: string[];
  warnings: string[];
};

export type OntologyAlignmentSummary = {
  ontologyId: string;
...
export type CrossFamilyTransferSummary = {
...
export type FusionQualitySummary = {
  profileHash: string;
...
export type SemanticMemoryQualitySummary = {
  versionId: string;
  memoryHash: string;
  entryCount: number;
...
export type GlobalSemanticGraphQualitySummary = {
  versionId: string;
  graphHash: string;
...
export type CarrierAdapterQualitySummary = {
  versionId: string;
  adapterHash: string;
  carrierCount: number;
  mappingCount: number;
...
export type UnderwritingRuleQualitySummary = {
  versionId: string;
  rulesHash: string;
  outcomeHash: string;
...
export type RiskFactorQualitySummary = {
  versionId: string;
...
export type RiskScoringQualitySummary = {
  versionId: string;
  scoringHash: string;
...
export type UnderwritingDecisionQualitySummary = {
  versionId: string;
  decisionHash: string;
  outcome: string;
...
export type SubmissionQualitySummary = {
  packageId: string;
  packageHash: string;
  carrierId: string;
  status: string;
...
export type CalibrationDistributionBucket = {
  bucket: string;
  count: number;
};

export type CalibrationCodeProfile = {
  acordCode: string;
  fields: number;
  ...
  rationaleDisagreementRate: number;
};

export type CalibrationFamilyBias = {
  formFamily: string;
  fields: number;
  avgConfidence: number;
...
export type CalibrationProfileDrift = {
  hasDrift: boolean;
...
export type CalibrationInsights = {
  ...
  thresholdAdjustedRankingQuality: number;
  rationaleWeightedSignalStability: number;
  confidenceDistribution: CalibrationDistributionBucket[];
  lowConfidenceFields: string[];
  borderlineFields: string[];
  unstableSignals: string[];
  perCodeProfiles: CalibrationCodeProfile[];
  biasByFormFamily: CalibrationFamilyBias[];
  lineage: import("../types").CalibrationLineageEntry[];
  suggestedOverrides: import("../types").CalibrationOverrideSuggestion[];
  profileDrift: CalibrationProfileDrift;
};

export type MappingQualityDrift = {
  key: string;
  baselineValue: string | number | null;
  currentValue: string | number | null;
};

export type MappingQualityReport = {
  fixtureId: string;
  generatedAt: string;
  ontology: OntologyAlignmentSummary;
  transferability: CrossFamilyTransferSummary;
  fusion: FusionQualitySummary;
  memory: SemanticMemoryQualitySummary;
  globalGraph: GlobalSemanticGraphQualitySummary;
  carrierAdapters: CarrierAdapterQualitySummary;
  underwritingRules: UnderwritingRuleQualitySummary;
  riskFactors: RiskFactorQualitySummary;
  riskScoring: RiskScoringQualitySummary;
  underwritingDecision: UnderwritingDecisionQualitySummary;
  submission: SubmissionQualitySummary;
  formFamily: import("../types").FormFamilyMetadata;
  ...
  totals: {
    ...
    topNHits: number;
    topNAccuracy: number;
    chosenWithinTopN: number;
    chosenWithinTopNRate: number;
    associationCorrect: number;
    associationAccuracy: number;
  };
  rationaleSignalDistribution: {
    embedding: { count: number; avg: number; min: number; max: number };
    lexical: { count: number; avg: number; min: number; max: number };
    dictionary: { count: number; avg: number; min: number; max: number };
    heuristic: { count: number; avg: number; min: number; max: number };
  };
  artifactHashes: {
    mappingDecisionHash: string;
    uiSchemaHash: string;
    jsonSchemaHash: string;
    xmlHash: string;
  };
  fields: MappingFieldQuality[];
  calibration: CalibrationInsights;
  drift: {
    hasDrift: boolean;
    differences: MappingQualityDrift[];
  };
};
...
export const DEFAULT_CALIBRATION_SIGNAL_WEIGHTS: CalibrationSignalWeights = {
  embedding: 0.5,
  lexical: 0.25,
  dictionary: 0.2,
  heuristic: 0.05,
...
export const DEFAULT_CALIBRATION_THRESHOLDS: ReviewConfidenceThresholds = {
  accepted: 0.8,
  review: 0.6,
  rejected: 0.45,
};
...
export function createDefaultCalibrationProfile(
  ...
  return {
    ...
    createdAt: now,
    updatedAt: now,
    globalThresholds: DEFAULT_CALIBRATION_THRESHOLDS,
    codeThresholdOverrides: {},
    signalWeights: DEFAULT_CALIBRATION_SIGNAL_WEIGHTS,
    familyOverrides: {},
    lineage: [],
    suggestedOverrides: [],
    auditTrail: [
...
function isAssociationInternallyConsistent(
  payload: MappingPersistencePayload,
...
function findExpectationForRecord(
  record: MappingReviewRecord,
  expectations: FixtureExpectations | undefined,
): FixtureFieldExpectation | undefined {
  if (!expectations) {
    return undefined;
  }
...
function buildMappingDecisionHash(payload: MappingPersistencePayload): string {
    ...
    payload.mappings
      .map((record) => {
        ...
        return {
          ...
          chosenCandidateCode:
            node?.chosenCandidateCode || record.mapping.chosen?.acordCode || null,
...
function resolveCalibrationProfile(input: EvaluateQualityInput): CalibrationProfile {
  return (
...
function resolveThresholdForCode(
  profile: CalibrationProfile,
  acordCode: string | undefined,
): ReviewConfidenceThresholds {
  if (!acordCode) {
...
function isRationaleDisagreement(
  lexical: number,
  embedding: number,
  dictionary: number,
  heuristic: number,
  ...
  return (
    lexicalEmbeddingGap > 0.4 || dictionaryEmbeddingGap > 0.45 || lexicalHeuristicGap > 0.5
  );
}
...
function buildCalibrationProfileHash(profile: CalibrationProfile): string {
  return hashString(
    stableSerialize({
...
function compareReports(
  baseline: MappingQualityReport | undefined,
  current: MappingQualityReport,
): MappingQualityDrift[] {
  if (!baseline) {
    return [];
  }

  const baselineCalibration = (baseline as Partial<MappingQualityReport>).calibration;
  ...
  const checks: Array<{
    ...
    },
    {
      key: "ontology.valid",
      baselineValue:
        baseline.ontology?.valid === undefined
          ? null
...
export function evaluateMappingQuality(input: EvaluateQualityInput): MappingQualityReport {
  ...
  const inferredFamily = payload.formFamily || inferFormFamily(payload, fixtureId);
  const familyId = input.familyId || input.expectations?.familyId || inferredFamily.familyId;
  const familyCalibration = resolveFamilyCalibration(calibrationProfile, familyId);
  const effectiveCalibrationProfile: CalibrationProfile = {
    ...calibrationProfile,
  ...
  const embeddingSignals: number[] = [];
  const lexicalSignals: number[] = [];
  const dictionarySignals: number[] = [];
  const heuristicSignals: number[] = [];
  const confidenceBuckets = new Map<string, number>();
  const normalizedConfidenceValues: number[] = [];
  const normalizationDeltas: number[] = [];
  const normalizationFactors: number[] = [];
  ...
  >();
  const familyProfiles = new Map<string, { confidences: number[]; belowThresholdCount: number }>();

  const fields: MappingFieldQuality[] = payload.mappings
    .map((record) => {
      ...
      const orderedCandidates = [...record.mapping.suggestions].sort(
        (left, right) =>
      ...
      const chosenCandidate =
        orderedCandidates.find((candidate) => candidate.acordCode === chosenCode) ||
      ...
      const normalizedSignals = normalizeSignalsForFamily(
        {
          embedding,
          lexical,
          dictionary,
      ...
      const ontologyConstraintViolations =
        chosenCandidate?.ontology?.violatedConstraints || [];
  ...
  const calibratedAccepted = fields.filter(
    (field) => field.chosenAcordCode && field.chosenConfidence >= field.appliedThreshold,
  ).length;
  ...
  const weightedStability = fields.length
    ? fields.reduce((sum, field) => {
        ...
        const record = payload.mappings.find(
          (item) => item.extractionBlockId === field.extractionBlockId,
        );
        const candidate = record?.mapping.suggestions.find(
          (item) => item.acordCode === field.chosenAcordCode,
        );
        const weightedConfidence =
          (candidate?.semanticSimilarity || 0) * normalizedWeights.embedding +
  ...
  const coverage: CoverageMetrics = {
    ...
    calibratedTopNDelta:
      baselineTopNAccuracy === null
        ? null
        : toFixedNumber(calibratedTopNAccuracy - baselineTopNAccuracy),
  };

  const confidenceDistribution: CalibrationDistributionBucket[] = Array.from(confidenceBuckets.entries())
    .map(([bucket, count]) => ({ bucket, count }))
    .sort((left, right) => left.bucket.localeCompare(right.bucket));

  const perCodeProfiles: CalibrationCodeProfile[] = Array.from(codeProfiles.entries())
    .map(([acordCode, aggregate]) => ({
      ...
      rationaleDisagreementRate: toFixedNumber(
        aggregate.disagreements / aggregate.confidences.length,
      ),
    }))
    .sort((left, right) => left.acordCode.localeCompare(right.acordCode));

  const biasByFormFamily: CalibrationFamilyBias[] = Array.from(familyProfiles.entries())
    .map(([formFamily, aggregate]) => ({
      formFamily,
      fields: aggregate.confidences.length,
      avgConfidence: safeAverage(aggregate.confidences),
      belowThresholdRate: toFixedNumber(
        aggregate.belowThresholdCount / aggregate.confidences.length,
      ),
    }))
  ...
  const ontologyMetadata = payload.ontologyAlignment || getDefaultOntologyMetadata();
  const selectedCodes = fields
    .map((field) => field.chosenAcordCode)
    .filter((code): code is string => Boolean(code));
  const ontologyValidation = validateOntologySelection(selectedCodes);
  const transferReport = evaluateCrossFamilyNormalization([
    { fixtureId, payload },
  ]);
  const transferability = {
    ...
    confidenceDrift: transferReport.driftByFamily.length
      ? safeAverage(
          transferReport.driftByFamily.map((item) => item.normalizedConfidenceDelta),
        )
      : 0,
    ...
    candidateStabilityDrift: transferReport.driftByFamily.length
      ? safeAverage(
          transferReport.driftByFamily.map((item) => item.candidateStabilityDelta),
        )
      : 0,
  };
  const fusionReport = evaluateSemanticFusion([
    {
      fixtureId,
      payload,
    },
  ...
  const fusedConfidenceScore = fusionReport.profiles.length
    ...
    : 0;
  const semanticMemory = buildSemanticMemorySnapshot(
    [
      {
        fixtureId,
        payload,
  ...
  const memoryRecommendations = generateSemanticMemoryRecommendations(
    fusionReport,
    semanticMemory,
  );
  const prioritizedConflicts = prioritizeConflictRecurrence(fusionReport, semanticMemory);
  const memoryDrift = evaluateLongitudinalMemoryDrift(
    input.baseline?.memory
      ? {
  ...
  const globalGraph = buildGlobalSemanticGraph(
    [
      {
        fixtureId,
        payload,
  ...
  const graphInference = evaluateGraphInference(payload, globalGraph.snapshot);
  const globalGraphDrift = evaluateGlobalSemanticDrift(
    payload.globalSemanticGraphSnapshot,
    globalGraph.snapshot,
  );
  const carrierAdapterMappings = evaluateCarrierAdapterMappings(payload, {
    familyId,
    overrides: payload.carrierAdapterOverrides || [],
  });
  const carrierSnapshot = buildCarrierAdapterSnapshot(
    [
      {
        fixtureId,
        payload,
  ...
  const carrierDrift = evaluateCarrierAdapterDrift(
    payload.carrierAdapterSnapshot,
    carrierSnapshot,
  );
  const underwritingReport = evaluateUnderwritingRules(payload, {
    familyId,
    overrides: payload.underwritingRuleOverrides || [],
  });
  const underwritingSnapshot = buildUnderwritingRuleSnapshot(payload, {
    previousSnapshot: payload.underwritingRuleSnapshot,
    decisions: payload.underwritingRuleDecisions || [],
    overrides: payload.underwritingRuleOverrides || [],
  });
  const underwritingDrift = evaluateUnderwritingRuleDrift(
    payload.underwritingRuleSnapshot,
    underwritingSnapshot,
  );
  const riskSignals = extractRiskFactors(payload, {
    overrides: payload.riskFactorOverrides || [],
  });
  const riskFactorSnapshot = buildRiskFactorSnapshot(payload, {
    previousSnapshot: payload.riskFactorSnapshot,
    overrides: payload.riskFactorOverrides || [],
  });
  const riskScoringBreakdown = evaluateRiskScoring(payload, {
    signals: riskSignals,
  });
  ...
  const underwritingDecisionReport = evaluateUnderwritingDecision(payload, {
    riskSignals,
    riskScoring: riskScoringBreakdown,
    overrides: payload.underwritingDecisionOverrides || [],
  });
  const underwritingDecisionSnapshot = buildUnderwritingDecisionSnapshot(payload, {
  ...
  const underwritingDecisionDrift = evaluateUnderwritingDecisionDrift(
    {
      riskFactorSnapshot: input.baseline?.riskFactors
        ? {
            ...
            categoryCounts: input.baseline.riskFactors.categoryCounts as any,
  ...
  const submissionDrift = evaluateSubmissionDrift(
    {
  ...
  const report: MappingQualityReport = {
    fixtureId,
    ...
    memory: {
      ...
      highPriorityRecommendations: memoryRecommendations.filter(
        (recommendation) => recommendation.priority === "high",
      ).length,
      highSeverityConflicts: prioritizedConflicts.filter(
        (conflict) => conflict.priority === "high",
      ).length,
      longitudinalDrift: {
    ...
    submission: {
      ...
      carrierId:
        payload.submissionPackage?.carrierPayloads?.[0]?.carrierId ||
    ...
    formFamily: {
      ...
      familyId,
      familyLabel:
        familyId === inferredFamily.familyId ? inferredFamily.familyLabel : familyId,
      evidence:
        familyId === inferredFamily.familyId
    ...
    calibration: {
      ...
      biasByFormFamily,
      lineage: familyCalibration.lineage,
      suggestedOverrides: suggestFamilyCalibrationOverrides(
        payload,
  ...
  report.calibration.profileDrift = {
    hasDrift:
      Boolean(input.baseline?.calibration?.profileHash) &&
...
export type XmlSemanticNode = {
  tag: string;
  value: string;
  attributes: Record<string, string>;
};

export type XmlSemanticIssue = {
...
export type XmlSemanticValidationReport = {
  fixtureId: string;
  generatedAt: string;
  ...
  nodeCount: number;
  codes: string[];
  issues: XmlSemanticIssue[];
  crossFieldWarnings: string[];
  isValid: boolean;
};

export type XmlSemanticDiffReport = {
  hasDrift: boolean;
  differences: XmlSemanticIssue[];
  baselineSemanticHash: string;
  currentSemanticHash: string;
};

export type ConsistencyFieldSummary = {
  acordCode: string;
  documents: number;
  avgConfidence: number;
  minConfidence: number;
...
export type ConsistencyBiasSummary = {
  formFamily: string;
  sampleCount: number;
  avgConfidence: number;
  lowConfidenceRate: number;
...
export type MultiDocumentConsistencyReport = {
  generatedAt: string;
  ...
  crossFamilyDrift: {
    confidence: number;
    rationaleSignals: number;
    chosenCandidateStability: number;
  };
  fields: ConsistencyFieldSummary[];
  biasByFormFamily: ConsistencyBiasSummary[];
  lowConfidenceFields: string[];
...
type ConsistencyInputDocument = {
  fixtureId: string;
  payload: MappingPersistencePayload;
};

export type EvaluateConsistencyInput = {
  documents: ConsistencyInputDocument[];
  calibrationProfile?: CalibrationProfile;
};
...
function parseAcordXmlNodes(xml: string): XmlSemanticNode[] {
...
function inferExpectedType(acordCode: string): "boolean" | "number" | "date" | "string" {
...
function findAcceptedAcordCodes(payload: MappingPersistencePayload): string[] {
  const codes: string[] = [];
...
function classifyFormFamily(fixtureId: string): string {
...
export function validateAcordXmlSemantic(
  payload: MappingPersistencePayload,
  xml: string,
  fixtureId?: string,
): XmlSemanticValidationReport {
  const resolvedFixtureId = fixtureId || payload.documentId || "unknown-fixture";
  const nodes = parseAcordXmlNodes(xml);
  const issues: XmlSemanticIssue[] = [];
  const crossFieldWarnings: string[] = [];
  const acceptedCodes = findAcceptedAcordCodes(payload);
  const ontologyValidation = validateOntologySelection(acceptedCodes);
  ...
  const byTag = new Map<string, XmlSemanticNode[]>();
  ...
  const hasApplicantAddress = Array.from(byTag.keys()).some(
    (tag) => /applicant/i.test(tag) && /address|city|state|postal|zip/i.test(tag),
  );
  ...
  const semanticSeed = stableSerialize(
    nodes
      .map((node) => ({
        tag: node.tag,
        value: node.value,
  ...
  return {
    ...
    codes: Array.from(new Set(nodes.map((node) => node.tag))).sort((a, b) =>
      a.localeCompare(b),
    ),
    issues,
    crossFieldWarnings,
...
export function diffAcordXmlSemantics(
  baseline: XmlSemanticValidationReport,
  current: XmlSemanticValidationReport,
): XmlSemanticDiffReport {
  const differences: XmlSemanticIssue[] = [];
...
export function evaluateMultiDocumentConsistency(
  input: EvaluateConsistencyInput,
): MultiDocumentConsistencyReport {
  const calibrationProfile =
    input.calibrationProfile || input.documents[0]?.payload.calibrationProfile || createDefaultCalibrationProfile();
  ...
  for (const document of input.documents) {
    const formFamily =
      document.payload.formFamily?.familyId ||
      inferFormFamily(document.payload, document.fixtureId).familyId;
  ...
  const fields: ConsistencyFieldSummary[] = [];
  ...
  for (const [acordCode, entries] of codeMap.entries()) {
    ...
    const lowConfidenceRate = toFixedNumber(
      entries.filter((entry) => entry.confidence < thresholds.accepted).length / entries.length,
    );
  ...
  const biasByFormFamily: ConsistencyBiasSummary[] = Array.from(familyMap.entries())
    .map(([formFamily, values]) => ({
      formFamily,
      sampleCount: values.length,
      avgConfidence: safeAverage(values),
      lowConfidenceRate: toFixedNumber(
        values.filter((value) => value < calibrationProfile.globalThresholds.accepted).length / values.length,
      ),
    }))
    .sort((left, right) => left.formFamily.localeCompare(right.formFamily));

  const consistencyHash = hashString(
    stableSerialize({
      fields: fields
        .map((field) => ({
  ...
  const normalizationReport = evaluateCrossFamilyNormalization(input.documents);
  const crossFamilyDrift = {
    confidence: normalizationReport.driftByFamily.length
      ? safeAverage(
          normalizationReport.driftByFamily.map((item) =>
            Math.abs(item.normalizedConfidenceDelta),
          ),
        )
      : 0,
    rationaleSignals: normalizationReport.driftByFamily.length
      ? safeAverage(
          normalizationReport.driftByFamily.map((item) =>
            Math.abs(item.rationaleSignalDelta),
          ),
        )
      : 0,
    chosenCandidateStability: normalizationReport.driftByFamily.length
      ? safeAverage(
          normalizationReport.driftByFamily.map((item) =>
            Math.abs(item.candidateStabilityDelta),
          ),
        )
      : 0,
  };
  ...
  return {
    ...
    biasByFormFamily,
    lowConfidenceFields: Array.from(new Set(lowConfidenceFields)).sort((a, b) =>
      a.localeCompare(b),
    ),

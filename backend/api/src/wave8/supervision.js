...
exports.getWave8StructureSuppressionSignals = getWave8StructureSuppressionSignals;
...
const WAVE8_SYNTHETIC_SUPERVISION_RULES = [
    {
...
function getWave8StructureSuppressionSignals(text, familyId) {
    const normalized = normalizeText(text);
    const reasons = [];
    const kinds = new Set();
    let score = 0;
    if (!normalized) {
        return { matched: true, score: 0.2, reasons: ["empty_text"], kinds: ["empty"] };
    }
    const family = normalizeText(familyId || "");
    if (/^(page\s+)?\d+\s*(of\s*\d+)?$/.test(normalized) || /^\d+\s*\/\s*\d+$/.test(normalized)) {
        score += 0.45;
        reasons.push("page_number_or_pagination");
        kinds.add("footer");
    }
    if (/(application|form|schedule|supplement|declaration|endorsement|policy|coverage)/.test(normalized) && normalized.split(" ").length <= 6) {
        score += 0.22;
        reasons.push("form_or_section_header");
        kinds.add("header");
        kinds.add("section");
    }
    if (/(insured|producer|agent|applicant|carrier|company|broker|underwriter)/.test(normalized) && /[:\-]$/.test(text.trim())) {
        score += 0.28;
        reasons.push("party_header_or_stub");
        kinds.add("header");
    }
    if (/(logo|©|copyright|all rights reserved|confidential|proprietary)/.test(normalized)) {
        score += 0.4;
        reasons.push("branding_or_legal_footer");
        kinds.add("logo");
        kinds.add("footer");
    }
    if (/(instructions?|please note|complete this section|if yes|if no|explain|describe|list)/.test(normalized)) {
        score += 0.24;
        reasons.push("instructional_copy");
        kinds.add("instruction");
    }
    if (/(section|part|schedule|item|table of contents|summary)/.test(normalized) && normalized.split(" ").length <= 8) {
        score += 0.2;
        reasons.push("section_label");
        kinds.add("section");
    }
    if (/(phone|fax|email|www\.|http|address|street|city|state|zip)/.test(normalized) && normalized.split(" ").length <= 10) {
        score += 0.16;
        reasons.push("contact_or_header_metadata");
        kinds.add("metadata");
    }
    if (family.includes("contractor") && /(operations|work performed|business description|premises operations)/.test(normalized)) {
        score += 0.18;
        reasons.push("family_specific_section_marker");
        kinds.add("section");
    }
    if (/^[A-Z0-9\s&.,/-]{8,}$/.test(text.trim()) && text.trim() === text.trim().toUpperCase()) {
        score += 0.18;
        reasons.push("all_caps_banner");
        kinds.add("header");
    }

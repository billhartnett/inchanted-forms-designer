...
exports.inferCandidateRiskAndDecisionIntelligence = inferCandidateRiskAndDecisionIntelligence;
exports.extractRiskFactors = extractRiskFactors;
exports.buildRiskFactorSnapshot = buildRiskFactorSnapshot;
exports.evaluateRiskScoring = evaluateRiskScoring;
exports.buildRiskScoringSnapshot = buildRiskScoringSnapshot;
exports.evaluateUnderwritingDecision = evaluateUnderwritingDecision;
exports.buildUnderwritingDecisionSnapshot = buildUnderwritingDecisionSnapshot;
exports.evaluateUnderwritingDecisionDrift = evaluateUnderwritingDecisionDrift;
...
function inferRiskCategory(text, candidate) {
    const normalized = normalizeText(`${text} ${candidate.acordCode} ${candidate.label}`);
    if (/(loss|claim|incident|prior)/.test(normalized))
        return "loss-history";
    if (/(operation|work|class|naics|trade)/.test(normalized))
        return "operations";
    if (/(hazard|flammable|height|roof|electrical|chemical)/.test(normalized))
        return "hazard";
    if (/(exposure|sales|payroll|receipts|units|revenue)/.test(normalized))
        return "exposure";
    if (/(applicant|insured|entity|experience|years)/.test(normalized))
        return "applicant";
    return "supplemental";
}
function carrierModifierFromCandidate(candidate) {
...
function severityFromCandidate(candidate, blockText) {
...
function inferCandidateRiskAndDecisionIntelligence(candidate, options) {
    ...
    const completenessContribution = clamp01(1 - (candidate.ontology?.violatedConstraints?.length || 0) * 0.2);
    const projectedScore = clamp01(severity * 0.55 +
        (candidate.underwritingRules?.scoreDelta ? Math.abs(candidate.underwritingRules.scoreDelta) : 0) * 0.2 +
        carrierModifier * 0.25);
    const projectedOutcome = projectedScore >= 0.8
        ? "decline"
        : projectedScore >= 0.6
...
function extractRiskFactors(payload, options) {
...
function buildRiskFactorSnapshot(payload, options) {
    ...
    const nextVersion = (previous ? Number(previous.versionId.replace("risk-v", "")) || 0 : 0) + 1;
    const versionId = `risk-v${nextVersion}`;
    const riskFactorHash = hashString(stableSerialize(signals.map((signal) => ({
...
function evaluateRiskScoring(payload, options) {
    const signals = options?.signals || extractRiskFactors(payload);
    const exposureSeverity = safeAverage(signals.filter((item) => item.category === "exposure").map((item) => item.severity));
    const hazardSeverity = safeAverage(signals.filter((item) => item.category === "hazard").map((item) => item.severity));
    const requiredFields = payload.fields.filter((field) => field.metadata?.required).length;
    const satisfiedRequired = payload.fields.filter((field) => field.metadata?.required &&
        field.metadata?.acordCode &&
        (field.metadata?.confidenceScore || 0) >= 0.55).length;
    ...
    const weightedScore = clamp01(toFixed(exposureSeverity * 0.32 +
        hazardSeverity * 0.33 +
        submissionCompleteness * 0.2 +
        carrierModifierScore * 0.15));
    const classification = weightedScore >= 0.67 ? "high" : weightedScore >= 0.34 ? "medium" : "low";
...
function buildRiskScoringSnapshot(payload, options) {
    const scoring = evaluateRiskScoring(payload, { signals: options?.signals });
    const previous = options?.previousSnapshot;
    const nextVersion = (previous ? Number(previous.versionId.replace("risk-score-v", "")) || 0 : 0) + 1;
    const versionId = `risk-score-v${nextVersion}`;
    const scoringHash = hashString(stableSerialize({
...
function buildDecisionRules(payload, scoring) {
    ...
    const hasContradiction = normalizedCodes.some((code) => code.includes("yes")) &&
    ...
    const rules = [
        ...
        },
        {
            ...
            message: scoring.weightedScore >= 0.85
...
function evaluateUnderwritingDecision(payload, options) {
    ...
    const ruleReport = (0, underwritingRules_1.evaluateUnderwritingRules)(payload, {
        familyId: payload.formFamily?.familyId,
        overrides: payload.underwritingRuleOverrides || [],
    });
    const graphSnapshot = payload.globalSemanticGraphSnapshot ||
        (0, globalSemanticGraph_1.buildGlobalSemanticGraph)([{ fixtureId: payload.documentId || "unknown-document", payload }]).snapshot;
    const graphInference = (0, globalSemanticGraph_1.evaluateGraphInference)(payload, graphSnapshot);
    ...
    const hasBlockers = decisionRules.some((rule) => rule.severity === "blocker" && !rule.passed) ||
        carrierBlockers > 0;
    let outcome;
    const conditions = [];
...
function buildUnderwritingDecisionSnapshot(payload, options) {
    ...
    const previous = options?.previousSnapshot;
    const nextVersion = (previous ? Number(previous.versionId.replace("uw-v", "")) || 0 : 0) + 1;
    ...
    return {
        generatedAt: CANONICAL_GENERATED_AT,
        versionId,
        decisionHash: hashString(stableSerialize({
...
function evaluateUnderwritingDecisionDrift(baseline, current) {
    const differences = [
        {
            key: "riskFactorHash",

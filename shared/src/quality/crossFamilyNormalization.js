...
exports.getFamilyNormalizationFactor = getFamilyNormalizationFactor;
exports.normalizeSignalsForFamily = normalizeSignalsForFamily;
exports.blendNormalizedSignals = blendNormalizedSignals;
exports.evaluateCrossFamilyNormalization = evaluateCrossFamilyNormalization;
...
function normalizeSignal(value, familyFactor) {
    const centered = clamp01(value);
    const shifted = 0.5 + (centered - 0.5) * familyFactor;
    return quantize(clamp01(shifted));
}
function getFamilyNormalizationFactor(familyId) {
    if (!familyId)
        return 1;
    return FAMILY_FACTORS[familyId] ?? 1;
}
function normalizeSignalsForFamily(raw, familyId) {
...
function blendNormalizedSignals(normalized, weights) {
    const total = Math.max(0.000001, weights.embedding + weights.lexical + weights.dictionary + weights.heuristic);
    const score = normalized.embedding * (weights.embedding / total) +
...
function evaluateCrossFamilyNormalization(documents) {
    ...
    for (const document of documents) {
        ...
        for (const record of document.payload.mappings) {
            const chosenCode = document.payload.decisionGraph.mappings[record.extractionBlockId]?.chosenCandidateCode ||
    ...
    const familySummaries = Array.from(byFamily.entries())
    ...
    const globalConfidence = average(familySummaries.map((item) => item.normalizedConfidenceAverage));
    const driftByFamily = familySummaries.map((summary) => ({
        ...
        rationaleSignalDelta: quantize(average([
        ...
        candidateStabilityDelta: quantize(Math.abs(summary.normalizedAverage.embedding - summary.normalizedAverage.lexical)),
    }));
    const transferabilityScore = familySummaries.length
        ? quantize(1 -
            average(driftByFamily.map((item) => Math.abs(item.normalizedConfidenceDelta))))
        : 0;
    return {
        familySummaries,

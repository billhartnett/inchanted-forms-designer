...
exports.normalizeCarrierResponse = normalizeCarrierResponse;
exports.buildSubmissionStatusSnapshot = buildSubmissionStatusSnapshot;
exports.evaluateSubmissionDrift = evaluateSubmissionDrift;
...
function selectCarrier(payload, overrides) {
    ...
    if (carrierFromAdapter)
        return carrierFromAdapter;
    return payload.formFamily?.familyId?.includes("contractor") ? "markel" : "anac";
}
function toCarrierPayload(payload, carrierId, acordXml) {
...
function buildSubmissionPackage(payload, options) {
    const overrides = options.overrides || payload.submissionOverrides || [];
    const carrierId = selectCarrier(payload, overrides);
    const riskSignals = (0, riskDecisionIntelligence_1.extractRiskFactors)(payload, { overrides: payload.riskFactorOverrides || [] });
    const riskScoring = (0, riskDecisionIntelligence_1.evaluateRiskScoring)(payload, { signals: riskSignals });
    ...
    const decision = (0, riskDecisionIntelligence_1.evaluateUnderwritingDecision)(payload, {
        riskSignals,
        riskScoring,
...
function normalizeCarrierResponse(input) {
    ...
    const normalized = {
        ...
        carrierReferenceId: typeof input.body.referenceId === "string" ? input.body.referenceId : null,
        ack: typeof input.body.ack === "boolean"
            ? input.body.ack
            : input.httpStatus >= 200 && input.httpStatus < 300,
        message: typeof input.body.message === "string" ? input.body.message : "carrier-response",
    };
    ...
    return {
        ...
        status,
        carrierReferenceId: typeof input.body.referenceId === "string" ? input.body.referenceId : undefined,
...
function buildSubmissionStatusSnapshot(input) {
...
function evaluateSubmissionDrift(baseline, current) {
    const differences = [
        {

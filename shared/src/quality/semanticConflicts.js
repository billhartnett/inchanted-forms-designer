...
exports.detectSemanticConflicts = detectSemanticConflicts;
exports.resolveSemanticConflicts = resolveSemanticConflicts;
exports.evaluateCrossOntologyConsistency = evaluateCrossOntologyConsistency;
...
function resolveCandidate(record) {
    const chosenCode = record.mapping.chosen?.acordCode;
    if (!chosenCode)
        return undefined;
    return (record.mapping.suggestions.find((item) => item.acordCode === chosenCode) ||
        record.mapping.chosen);
}
function scoreCandidate(candidate, calibration) {
    const weights = calibration.signalWeights;
    const total = Math.max(0.000001, weights.embedding + weights.lexical + weights.dictionary + weights.heuristic);
    const evidence = (candidate.semanticSimilarity || 0) * (weights.embedding / total) +
    ...
    return Number((((candidate.normalizedConfidenceScore || candidate.confidenceScore) * 0.7 + evidence * 0.3) *
        (1 - ontologyPenalty)).toFixed(6));
}
function buildConflictId(seed) {
...
function detectSemanticConflicts(payload, options) {
    ...
    const calibration = options?.calibrationProfile || payload.calibrationProfile || (0, evaluation_1.createDefaultCalibrationProfile)();
    const conflicts = [];
    ...
    for (const record of payload.mappings) {
        ...
        const disagreement = Math.abs((candidate.semanticSimilarity || 0) - (candidate.lexicalScore || 0)) > 0.45;
        ...
        const arbitration = (0, multiOntology_1.arbitrateCandidateByOntology)(candidate, bundles);
    ...
    for (const [normalizedText, entries] of byText.entries()) {
        if (!normalizedText || entries.length < 2)
            continue;
        const uniqueCodes = Array.from(new Set(entries.map((item) => item.candidate.acordCode))).sort((a, b) => a.localeCompare(b));
        if (uniqueCodes.length < 2)
            continue;
        const allHighConfidence = entries.every((item) => item.candidate.confidenceScore >= calibration.globalThresholds.review);
        ...
        conflicts.push({
            ...
            extractionBlockIds: entries.map((item) => item.record.extractionBlockId).sort((a, b) => a.localeCompare(b)),
    ...
    return conflicts.sort((left, right) => left.class.localeCompare(right.class) || left.conflictId.localeCompare(right.conflictId));
}
function resolveSemanticConflicts(payload, options) {
    const familyId = options?.familyId || payload.formFamily?.familyId || "unknown-family";
    const bundles = (0, multiOntology_1.selectOntologyBundlesForFamily)(familyId);
    ...
    const graphSnapshot = options?.graphSnapshot ||
    ...
    const decisions = conflicts.map((conflict) => {
        ...
        for (const extractionBlockId of conflict.extractionBlockIds) {
            ...
            const graphInference = (0, globalSemanticGraph_1.inferCandidateFromGlobalGraph)(candidate, graphSnapshot) || {
                inferred: false,
                confidence: 0,
                evidence: [],
                recommendedCodes: [],
            ...
            const underwritingRules = candidate.underwritingRules ||
                (0, underwritingRules_1.evaluateCandidateUnderwritingRules)(candidate, {
            ...
            const score = Number((baseScore * 0.72 +
                arbitration.score * 0.12 +
                graphInference.confidence * 0.08 +
                resolvedUnderwriting.scoreDelta * 0.08 -
                (resolvedUnderwriting.hardFailures.length > 0 ? 0.2 : 0)).toFixed(6));
            candidates.push({
                extractionBlockId,
                candidate: {
                    ...candidate,
        ...
        return {
            ...
            lineage: [
                ...
                ...(winner
                    ? ((0, globalSemanticGraph_1.inferCandidateFromGlobalGraph)(winner.candidate, graphSnapshot)?.lineage || [])
                    : []),
            ],
        };
    });
    ...
    const conflictHash = hashString(stableSerialize({
...
function evaluateCrossOntologyConsistency(documents) {
    const namespaceTotals = new Map();
    for (const document of documents) {
        const bundles = (0, multiOntology_1.selectOntologyBundlesForFamily)(document.payload.formFamily?.familyId);
    ...
    const globalAvg = averages.length
        ...
        : 0;
    const confidenceDrift = Number((averages.reduce((sum, value) => sum + Math.abs(value - globalAvg), 0) /
        Math.max(1, averages.length)).toFixed(6));
    return {
        generatedAt: new Date().toISOString(),

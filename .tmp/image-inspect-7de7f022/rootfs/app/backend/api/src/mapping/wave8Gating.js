...
exports.applyWave8Gating = applyWave8Gating;
function toNumber(value, fallback) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : fallback;
}
function applyWave8Gating(mappings, options) {
    const enabled = options?.enabled ?? process.env.WAVE8_GATING_ENABLED !== "0";
    if (!enabled) {
        return mappings;
    }
    const minConfidence = Math.max(0, Math.min(1, options?.minConfidence ?? toNumber(process.env.WAVE8_GATING_MIN_CONFIDENCE, 0.2)));
    const retainTopSuggestions = Math.max(1, Math.min(10, Math.floor(options?.retainTopSuggestions ?? toNumber(process.env.WAVE8_GATING_TOP_K, 5))));

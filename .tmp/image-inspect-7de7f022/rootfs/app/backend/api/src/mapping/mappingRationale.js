...
exports.buildMappingRationale = buildMappingRationale;
...
function buildMappingRationale(mapping) {
    ...
    return {
        ...
        arbitrationEvidence: chosen?.arbitration
            ? stableUnique([
                `Namespace ${chosen.arbitration.winningNamespace}`,
                chosen.arbitration.reason,
            ])
            : [],
        ...
        riskScoringEvidence: chosen?.riskFactors
            ? stableUnique([
                `Completeness contribution ${chosen.riskFactors.completenessContribution.toFixed(3)}`,
            ])
            : [],

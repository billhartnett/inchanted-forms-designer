...
function main() {
  ...
  const validationReport = {
    schemaVersion: 'corpus-validation-v10.v1',
    generatedAt: new Date().toISOString(),
    repositoryRoot: repoRoot,
    source: {
      engine: 'semantic-engine-v2',
      patchSet: 'phase8-next-rule-synthesis-v10',
  ...
  const outputRoot = path.join(repoRoot, 'corpus-validation-v10');
  ...
  const snapshotManifest = {
    schemaVersion: 'corpus-validation-v10-snapshots.v1',
    generatedAt: new Date().toISOString(),
  ...
  fs.writeFileSync(path.join(outputRoot, 'README.md'), [
    '# Corpus Validation v10',
    '',
    'Generated from the next Phase 8 semantic rule synthesis cycle covering table boundary detection, grouping logic, suppression refinement, and unmapped fillable-field resolution.',
    '',
    ...
    `- Readiness: ${overallReadiness}`,
    '',
    'This validation reflects the next deterministic refinement pass against the merged XFDL/PDF corpus for Phase 8.',
    '',
  ].join('\n'), 'utf8');
}

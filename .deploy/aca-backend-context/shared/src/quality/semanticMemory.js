...
exports.buildSemanticMemorySnapshot = buildSemanticMemorySnapshot;
exports.updateSemanticMemoryFromFusion = updateSemanticMemoryFromFusion;
exports.prioritizeConflictRecurrence = prioritizeConflictRecurrence;
exports.generateSemanticMemoryRecommendations = generateSemanticMemoryRecommendations;
exports.evaluateLongitudinalMemoryDrift = evaluateLongitudinalMemoryDrift;
...
function recurrenceWeight(conflictClass) {
    if (conflictClass === "cross-ontology-disagreement")
        return 1;
    if (conflictClass === "cross-family-disagreement")
        return 0.7;
    return 0.45;
}
...
function applyDecisionsToSnapshot(snapshot, decisions) {
    if (!decisions.length) {
        return snapshot;
    }
    const sortedDecisions = [...decisions].sort((left, right) => left.decisionId.localeCompare(right.decisionId));
    ...
    return {
        ...
        lineage: mergeLineage([
            ...snapshot.lineage,
            ...sortedDecisions.map((decision) => `memory.decision:${decision.action}:${decision.targetVersionId || "none"}:${decision.groupId || "all"}`),
        ]),
    };
}
function buildSemanticMemorySnapshot(documents, options) {
...
function updateSemanticMemoryFromFusion(fusion, options) {
    const previous = options?.previousSnapshot;
    const previousByGroup = new Map((previous?.entries || []).map((entry) => [entry.groupId, entry]));
    const conflictWeightByGroup = new Map();
    for (const conflict of fusion.conflicts) {
        const current = conflictWeightByGroup.get(conflict.groupId) || 0;
        conflictWeightByGroup.set(conflict.groupId, toFixed(current + recurrenceWeight(conflict.class)));
    }
    const entries = fusion.profiles
        .map((profile) => {
        ...
        const nextRecurrence = toFixed(previousRecurrence + (conflictWeightByGroup.get(profile.groupId) || 0));
        const confidenceStability = nextConfidenceHistory.length <= 1
            ? 1
            : 1 -
                clamp01(Math.abs(nextConfidenceHistory[nextConfidenceHistory.length - 1] -
...
function prioritizeConflictRecurrence(fusion, snapshot) {
    const recurrenceByGroup = new Map((snapshot?.entries || []).map((entry) => [entry.groupId, entry.conflictRecurrence]));
    return fusion.conflicts
        .map((conflict) => {
        const recurrence = recurrenceByGroup.get(conflict.groupId) || 0;
        const severityScore = toFixed(clamp01(recurrenceWeight(conflict.class) * 0.6 +
            Math.min(1, recurrence / 10) * 0.4));
        const priority = severityScore >= 0.75 ? "high" : severityScore >= 0.45 ? "medium" : "low";
        return {
        ...
        .sort((left, right) => right.severityScore - left.severityScore || left.conflictId.localeCompare(right.conflictId));
}
function generateSemanticMemoryRecommendations(fusion, snapshot) {
    ...
    const conflictScoreByGroup = new Map(prioritizedConflicts.map((conflict) => [conflict.groupId, conflict.severityScore]));
    return fusion.profiles
        ...
        const confidence = toFixed(clamp01(profile.fusedConfidenceScore * 0.65 + memoryStability * 0.35 - conflictSeverity * 0.2));
        const priority = confidence < 0.55 || conflictSeverity >= 0.7
            ? "high"
...
function evaluateLongitudinalMemoryDrift(baseline, current) {
    if (!baseline) {
        return {
            hasDrift: false,
    ...
    const baselineConfidence = average(baseline.entries.map((entry) => entry.confidenceHistory[entry.confidenceHistory.length - 1] || 0));
    const currentConfidence = average(current.entries.map((entry) => entry.confidenceHistory[entry.confidenceHistory.length - 1] || 0));
    const baselineRecurrence = toFixed(baseline.entries.reduce((sum, entry) => sum + entry.conflictRecurrence, 0));
    const currentRecurrence = toFixed(current.entries.reduce((sum, entry) => sum + entry.conflictRecurrence, 0));
    ...
    return {
        ...
        driftHash: hashString(stableSerialize({

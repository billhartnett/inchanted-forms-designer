...
function findField(
  ...
  return payload.fields.find(
    (field) => field.metadata?.extractionBlockId === extractionBlockId,
  );
}
...
export function buildAcordXmlFromPayload(
  ...
  lines.push("<ACORD>");
  lines.push(
    `  <Submission generatedAt="${escapeXml(generatedAt)}" documentId="${escapeXml(
  ...
  for (const record of nodeRecords) {
    ...
    if (
      ...
      ontologyIssues.push(
        `Skipped ${chosen.acordCode} due to mutual exclusivity conflict.`,
      );
      continue;
    }
    ...
    lines.push(
      `    <${tag} label="${label}" fieldId="${escapeXml(field.id)}" extractionBlockId="${escapeXml(
        extractionBlockId,
      )}" source="${source}" confidence="${chosen.confidenceScore.toFixed(3)}" fusedConfidence="${(
        chosen.normalizedConfidenceScore || chosen.confidenceScore
        ...
        typeof field.pageIndex === "number" ? field.pageIndex : ""
      }" x="${formatNumber(field.x)}" y="${formatNumber(field.y)}" width="${formatNumber(field.width)}" height="${formatNumber(field.height)}" rotation="${formatNumber(
        field.rotation,
      )}">${value}</${tag}>`,
    );

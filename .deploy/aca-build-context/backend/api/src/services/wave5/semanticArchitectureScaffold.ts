...
export type MultiTokenSemanticWindow = {
  startIndex: number;
  endIndex: number;
  tokens: string[];
  windowId: string;
...
export type SemanticLatticeNode = {
  nodeId: string;
  token: string;
  layer: number;
};

export type SemanticLatticeEdge = {
  fromNodeId: string;
  toNodeId: string;
  edgeType: "window" | "neighbor";
};

export type SemanticLattice = {
  nodes: SemanticLatticeNode[];
  edges: SemanticLatticeEdge[];
};

export type CarrierSemanticPrior = {
  concept: string;
  weight: number;
};

export type CarrierSemanticPriorMap = Record<string, CarrierSemanticPrior[]>;

export type SemanticContextScoringInput = {
  candidateConcept: string;
  tokenOverlapScore: number;
  neighborhoodCoverage: number;
  latticeConnectivity: number;
  carrierPriorWeight?: number;
  contextBoost?: number;
};

export type SemanticGeometryFusionInput = {
...
export type SemanticGeometryFusionOutput = {
  fusedScore: number;
...
export function buildSemanticNeighborhoods(
  tokens: string[],
  radius = 2,
  ...
  const neighborhoods: SemanticNeighborhood[] = [];
...
export function buildMultiTokenSemanticWindows(
  tokens: string[],
  minWindowSize = 2,
  maxWindowSize = 4,
): MultiTokenSemanticWindow[] {
  const normalized = tokens.map(normalizeToken).filter((token) => token.length > 0);
  ...
  const windows: MultiTokenSemanticWindow[] = [];
...
export function buildSemanticLattice(
  neighborhoods: SemanticNeighborhood[],
  windows: MultiTokenSemanticWindow[],
): SemanticLattice {
  const nodeMap = new Map<string, SemanticLatticeNode>();
  const edges: SemanticLatticeEdge[] = [];
  ...
  const nodes = [...nodeMap.values()].sort(
    (left, right) => left.layer - right.layer || left.nodeId.localeCompare(right.nodeId),
  );
...
export function resolveCarrierSpecificSemanticPriors(
  carrierId: string | undefined,
  globalPriors: CarrierSemanticPrior[],
  carrierPriorMap?: CarrierSemanticPriorMap,
): CarrierSemanticPrior[] {
  const normalizedCarrier = normalizeToken(carrierId || "");
  const carrierPriors =
    normalizedCarrier.length > 0 && carrierPriorMap && carrierPriorMap[normalizedCarrier]
  ...
  const byConcept = new Map<string, CarrierSemanticPrior>();
...
export function scoreContextAwareSemanticCandidate(
  input: SemanticContextScoringInput,
): number {
...
export function fuseSemanticAndGeometryScores(
  input: SemanticGeometryFusionInput,
): SemanticGeometryFusionOutput {
  ...
  const fusedScore = clamp01(
    clamp01(input.semanticScore) * semanticContribution +

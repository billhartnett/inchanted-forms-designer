...
import { getAcordSchemaEntries, type AcordDictionaryEntry } from "shared/acord";
...
export function initializeAcordDictionary(): DictionaryState {
  ...
  const schemaEntries = getAcordSchemaEntries();
  const mergedEntries = mergeAcordEntries(parsedEntries, schemaEntries);

  const byCode = new Map<string, AcordDictionaryEntry>();
  const indexed = mergedEntries.map((entry) => {

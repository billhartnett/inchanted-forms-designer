...
function main() {
  ...
  const validationReport = {
    schemaVersion: 'corpus-validation-v9.v1',
    generatedAt: new Date().toISOString(),
    repositoryRoot: repoRoot,
    source: {
      engine: 'semantic-engine-v2',
      patchSet: 'phase8-final-rule-synthesis-v9',
  ...
  const outputRoot = path.join(repoRoot, 'corpus-validation-v9');
  ...
  const snapshotManifest = {
    schemaVersion: 'corpus-validation-v9-snapshots.v1',
    generatedAt: new Date().toISOString(),
  ...
  fs.writeFileSync(path.join(outputRoot, 'README.md'), [
    ...
    `- Readiness: ${overallReadiness}`,
    '',
    'This validation reflects the final deterministic refinement pass against the merged XFDL/PDF corpus for Phase 8.',
    '',
  ].join('\n'), 'utf8');
}
